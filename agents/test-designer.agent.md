...
4. Never push a protected branch, merge, or approve. Publish the verified Journey test report only through `publish-agent-report`.

Verify and invoke `publish-agent-report` to commit/push the test report and its PR comment, then wait for human confirmation before updating `workflow.json`.
